...
module.exports.index = async (req, res, next) => {
    try {
        ...
        if (!cartId) {
            ...
            return ResponseFormatter.success(res, { cart: null, items: [] }, 'No cart found (no cartId cookie).');
        }
        ...
        if (!cart) {
            return ResponseFormatter.success(res, { cart: null, items: [] }, 'Cart not found in database.');
        }
...
module.exports.order = async (req, res, next) => {
    try {
        ...
        if (!cartId) {
            return ApiError(res, 'Missing cart ID in cookies.', 400);
        }
        ...
        if (!cart || cart.products.length === 0) {
            return ApiError(res, 'Cart is empty or not found.', 400);
        }
        ...
        for (const item of cart.products) {
            ...
            if (!product || product.deleted || product.status !== 'active') {
                return ApiError(res, `Product ${item.product_id || 'Unknown'} is not available.`, 400);
            }
        ...
        if (insufficientStock) {
            return ApiError(res, `Insufficient stock for product: ${invalidProductName}.`, 400);
        }
